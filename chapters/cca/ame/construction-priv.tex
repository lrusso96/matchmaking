\begin{construction}\label{constr:ame_nizk_priv}
    ...
    \begin{description}
        \item[$\setup(\secparam)$:] on input the security parameter $\secparam$, computes $(\msk', \mpk') \getsr \setup'(\secparam)$, and $\crs \getsr \init(\secparam)$. The algorithm outputs $\msk = \msk'$ as the master secret key and $\mpk = (\mpk', \crs)$ as the master public key. All other algorithms are implicitly given $\mpk$ as additional input.
